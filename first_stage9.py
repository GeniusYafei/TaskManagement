...
def view_tasks():
    ...
    # Use 'tasks' instead of 'tasks_data'
    for list_name, task_list in tasks.items():
        task_items = [p.li(
            ...
            p.form(
                ...
                # formaction=f"/task_content/{list_name}/{task['task_name']}"),
                p.input(type="submit", value="Delete",
                        formaction=f"/delete_task/{list_name}/{task['task_name']}"),
                p.input(type="submit", value="Complete",
                        formaction=f"/completed_task/{list_name}/{task['task_name']}")
                if not task.get('is_completed', False) else "Completed",
                method="POST"
            )
    ...
    add_task_form = p.form(
        ...
        p.label("Deadline:", for_="task_deadline"),p.br,
        p.input(type="date", id="task_deadline", name="task_deadline"),
        p.input(type="submit", value="Add Task"),
        method="POST", action="/add_task"
    )

    body_content.append(p.h2("Add a New Task:"))  # Corrected p.h2() usage
